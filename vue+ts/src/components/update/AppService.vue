<template>
  <div class="service-update-page">
    <div class="page-container">
      <h2 class="title">
        <i class="fa fa-edit icon"></i> 修改 Service - {{ service.name }}
      </h2>
      ...
      <div class="readonly-info">
        ...
        <p><strong>命名空间：</strong> {{ service.namespace }}</p>
      </div>
      ...
      <div class="main-content">
        ...
        <div class="form-content">
          ...
          <div class="form-section">
            <h3><i class="fa fa-tag section-icon orange"></i> Service Labels</h3>
            <div
              v-for="(label, idx) in service.labels"
              :key="'label-' + idx"
              class="form-item"
            >
              <label>Key：</label>
              <input v-model="label.key" type="text" />
              <label>Value：</label>
              <input v-model="label.value" type="text" />
          ...
          <!-- Selector (只读) -->
          <div class="form-section">
            <h3><i class="fa fa-filter section-icon green"></i> Selector</h3>
            <p class="labels-hint">⚠️ Selector 由 K8s 自动生成，不可修改</p>
            <div
          ...
          <!-- 类型 -->
          <div class="form-section">
            <h3><i class="fa fa-cogs section-icon purple"></i> Service 类型</h3>
            <div class="form-item">
              <label>类型：</label>
          ...
          <!-- 端口配置 -->
          <div class="form-section">
            <h3><i class="fa fa-plug section-icon teal"></i> 端口配置(不能添加端口，只能修改当前端口)</h3>
            <div
              v-for="(port, idx) in service.ports"
              :key="'port-' + idx"
              class="port-item"
            >
              <div class="port-row">
                ...
                <label>端口：</label>
                <input v-model="port.port" type="number" />
              </div>
              <div class="port-row">
                <label>TargetPort：</label>
                <input v-model="port.targetPort" type="number" />
                <button
...
export default defineComponent({
  name: "UpdateService",
  props: {
    svcData: { type: Object, required: true },
  },
  setup(props) {
    const router = useRouter();

    const service = ref({
      name: props.svcData.name,
      namespace: props.svcData.namespace,
      // 修改这里，labels 可编辑
      labels: Object.entries(props.svcData.labels || {}).map(([k, v]) => ({
        key: k,
        value: v,
      })),
      selector: props.svcData.selector || {},
      type: props.svcData.type || "ClusterIP",
     ports: (props.svcData.ports || []).map((p) => ({
  ...
  targetPort: Number(p.targetPort),
    ...
    const labelsObj = computed(() => {
      const obj = {};
      service.value.labels.forEach((l) => {
    ...
    const addPort = () =>
      service.value.ports.push({ protocol: "TCP", port: "", targetPort: "" });
    //   alert("Kubernetes 不允许在现有 ClusterIP Service 上新增端口，请删除后重新创建新的 Service。");
    const removePort = (idx) => service.value.ports.splice(idx, 1);

   const updateService = async () => {
  try {
    await axios.post("http://192.168.216.50:8090/service/UpdateService", {
      name: service.value.name,
      namespace: service.value.namespace,
      labels: labelsObj.value,
      type: service.value.type,
